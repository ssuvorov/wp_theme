...
function noo_organici_customizer_add_custom_controls( $wp_customize ) {

	class NOO_Organici_Customizer_Sub_Section extends WP_Customize_Control {
	...
	//
	class NOO_Organici_Customizer_Control_Checkbox extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			if ( isset( $args['data_on'] ) && !empty( $args['data_on'] ) ) {
				...
				$this->data_on = esc_html__( 'ON', 'noo-organici' );
			}
			
			if ( isset( $args['data_off'] ) && !empty( $args['data_off'] ) ) {
				$this->data_off = $args['data_off'];
			} else {
				$this->data_off = esc_html__( 'OFF', 'noo-organici' );
			}
		}
	...
	//
	class NOO_Organici_Customizer_Control_Switch extends NOO_Organici_Customizer_Control_Checkbox {
	...
	//
	class NOO_Organici_Customizer_Control_UI_Slider extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<label class="noo-control">
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<input id="noo-ui-slider-<?php echo esc_attr($this->id); ?>" type="text" <?php $this->link(); ?> class="noo-slider" value="<?php echo esc_textarea( $this->value() ); ?>" <?php echo ( $data_min . ' ' . $data_max . ' ' . $data_step ); ?>/>
			</label>
			<?php
		}
	}
	...
	//
	class NOO_Organici_Customizer_Control_Radio extends WP_Customize_Control {
		public $type = 'radio';
		public function render_content() {
	...
	//
	class NOO_Organici_Customizer_Control_Same_As_Radio extends WP_Customize_Control {
		public $type = 'radio';
		public function render_content() {
			if ( empty( $this->choices ) )
				return;
	...
	class NOO_Organici_Customize_Alpha_Color extends WP_Customize_Control {
	...
	class NOO_Organici_Customize_Image_Control extends WP_Customize_Image_Control {
		public $type = 'image';
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->add_tab( 'library',  esc_html__( 'Media Library', 'noo-organici' ), array( $this, 'media_library' ) );
		...
		public function media_library() {
			?>
				<a class="choose-from-library-link button" data-controller="<?php echo esc_attr($this->id); ?>">
					<?php echo esc_html__( 'Open Library', 'noo-organici' ); ?>
				</a>
			<?php
		}
	 ...
	 */
	class NOO_Organici_Customize_Settings_Upload extends WP_Customize_Control {
	 ...
	 */
	class NOO_Organici_Customize_Settings_Download extends WP_Customize_Control {
	...
	class NOO_Organici_Customizer_Control_Textarea extends WP_Customize_Control {
	...
	class NOO_Organici_Customizer_Control_Multiple_Select extends WP_Customize_Control {
	...
	class NOO_Organici_Customizer_Control_Divider extends WP_Customize_Control {
		public $type = 'divider';
		public function render_content() {
			echo '<hr/>';
		}
	...
	//
	class NOO_Organici_Customizer_Control_Widgets_Select extends WP_Customize_Control {
      ...
      <label>
        ...
        <select <?php $this->link(); ?>>
          <option value=""><?php echo esc_html__( '- Select Sidebar -', 'noo-organici' ); ?></option>
          <?php
	...
	// Pages Select.
	//
	class NOO_Organici_Customizer_Control_Pages_Select extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$dropdown = str_replace( '<select', '<select ' . $this->get_link() . 'class="noo-customize-chosen" data-placeholder="' . esc_html__( '- Select Page -', 'noo-organici' ) . '"', $dropdown );
		?>
      <label>
        <span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
        <?php echo $dropdown; ?>
	...
	// Posts Select.
	//
	class NOO_Organici_Customizer_Control_Posts_Select extends WP_Customize_Control {
	...
	// Terms Select.
	//
	class NOO_Organici_Customizer_Control_Terms_Select extends WP_Customize_Control {
	 ...
	 */
	class NOO_Organici_Customizer_Control_Google_Fonts extends WP_Customize_Control {
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			$this->weight = empty( $this->weight ) ? noo_organici_get_theme_default( 'font_weight' ) : $this->weight;
			...
			if ( empty ( self::$font_weight_name ) )
				self::$font_weight_name = array(
					'100'       => esc_html__( 'Extra Light', 'noo-organici' ),
					'100italic' => esc_html__( 'Extra Light Italic', 'noo-organici' ),
					'200'       => esc_html__( 'Light', 'noo-organici' ),
					'200italic' => esc_html__( 'Light Italic', 'noo-organici' ),
					'300'       => esc_html__( 'Book', 'noo-organici' ),
					'300italic' => esc_html__( 'Book Italic', 'noo-organici' ),
					'400'       => esc_html__( 'Regular', 'noo-organici' ),
					'400italic' => esc_html__( 'Regular Italic', 'noo-organici' ),
					'500'       => esc_html__( 'Medium', 'noo-organici' ),
					'500italic' => esc_html__( 'Medium Italic', 'noo-organici' ),
					'600'       => esc_html__( 'Semi-bold', 'noo-organici' ),
					'600italic' => esc_html__( 'Semi-bold Italic', 'noo-organici' ),
					'700'       => esc_html__( 'Bold', 'noo-organici' ),
					'700italic' => esc_html__( 'Bold Italic', 'noo-organici' ),
					'800'       => esc_html__( 'Extra-bold', 'noo-organici' ),
					'800italic' => esc_html__( 'Extra-bold Italic', 'noo-organici' ),
					'900'       => esc_html__( 'Heavy', 'noo-organici' ),
					'900italic' => esc_html__( 'Heavy Italic', 'noo-organici' ),
				);
		}
		...
		?>
          ...
          <div class="noo-font-selects">
            <select class="noo-font-family noo-customize-chosen" data-placeholder="<?php echo esc_html__( '- Font Family -', 'noo-organici' ); ?>" <?php $this->link(); ?>>
            <option value=""></option>
              <?php
				$selected_font_id = 0;
		...
		?>
            ...
            <select class="noo-font-weight-and-style">
              <option value=""><?php echo esc_html__( '- Styles -', 'noo-organici' ); ?></option>
              <?php
		...
		?>
            ...
            <select class="noo-font-subset" data-customize-setting-link="<?php echo esc_attr($this->id) . '_subset'; ?>">
              <option value=""><?php echo esc_html__( '- Subsets -', 'noo-organici' ); ?></option>
              <?php
		...
		public function get_fonts( $amount = 30 ) {
			$fontFile = get_template_directory_uri() . '/includes/framework/customizer/data/google-web-fonts.txt';
			$content = json_decode( wp_remote_retrieve_body( wp_remote_get( $fontFile ) ) );
	...
	class NOO_Organici_Customizer_Control_Font_Size extends WP_Customize_Control {
		public $type = 'select';
	...
	class NOO_Organici_Customizer_Control_MailChimp_Mail_List extends WP_Customize_Control {
		public $type = 'noo-mc-mail-list';
		...
		public function render_content() {
			...
			<label>
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<input <?php $this->link(); ?> class="regular-text" type="text" placeholder="<?php echo esc_html__( 'Mailchimp API key', 'noo-organici'); ?>" value="<?php echo esc_attr( $this->value() ); ?>"/>
				<p class="description"><?php echo sprintf( noo_organici_kses( __( 'Enter Mailchimp API key to connect to your account. Don\'t know where to find? <a href="%s" target="_BLANK">Just click here to get your API</a>', 'noo-organici') ), 'https://us5.admin.mailchimp.com/account/api/'); ?></p>
			</label>
...
add_action( 'customize_register', 'noo_organici_customizer_add_custom_controls' );