 ...
 * @since 1.0.0
 */
class Noo_Organici_Meta_Boxes_Helper {
	public $prefix  = 'noo-organici';
	public $page  = 'post';
	...
	public function add_meta_box( $meta_box, $template = '', $template_data = array() ) {
		...
		if ( !isset( $meta_box['fields'] ) || empty( $meta_box['fields'] ) ) {
			...
			case 'data-video':
				$meta_box['fields'][] = array(
					...
					'label' => esc_html__( 'M4V File URL', 'noo-organici' ),
					'desc' => esc_html__( 'Place the URL to your .m4v video file here.', 'noo-organici' ),
					'type' => 'text',
				);
				$meta_box['fields'][] = array(
					'id' => $this->prefix . '_video_ogv',
					'label' => esc_html__( 'OGV File URL', 'noo-organici' ),
					'desc' => esc_html__( 'Place the URL to your .ogv video file here.', 'noo-organici' ),
					'type' => 'text',
				);
				$meta_box['fields'][] = array(
				...
				$meta_box['fields'][] = array(
					...
					'desc' => esc_html__( 'If you are using something other than self hosted video (YouTube, Vimeo, etc.) paste the embed code here. This field will override the above settings.', 'noo-organici' ),
					'type' => 'textarea',
					'std'  => ''
				);
				break;
			...
			case 'data-image':
				$meta_box['fields'][] = array(
					...
					'label' => esc_html__( 'Upload Image', 'noo-organici' ),
					'type' => 'image',
				);
				break;
		...
		$callback = create_function( '$post,$meta_box', 'noo_organici_create_meta_box( $post, $meta_box["args"] );' );
